...
# Change the threshold of detection according to what else is inside the image
...
image = cv2.imread(r'path_to\img.png')


templates = [
    Template(image_path=r'path_to\template1.png', label="1", color=(0, 0, 255)),
    Template(image_path=r'path_to\template2.png', label="2", color=(0, 255, 0)),
    Template(image_path=r'path_to\template3.png', label="3", color=(0, 191, 255)),
]

##### DETECTING OBJECT WITH TEMPLATE MATCHING

def detectAndSave(filter, index):
    ...
    # save the image
    cv2.imwrite("path\\" + index + ".png",
    image_with_detections)

    ##### REMOVE DUPLICATES

detectAndSave(cv2.TM_CCOEFF,"TM_CCOEFF")
detectAndSave(cv2.TM_CCOEFF_NORMED,"TM_CCOEFF_NORMED")
detectAndSave(cv2.TM_CCORR_NORMED,"TM_CCORR_NORMED")
detectAndSave(cv2.TM_SQDIFF,"TM_SQDIFF")
detectAndSave(cv2.TM_SQDIFF_NORMED,"TM_SQDIFF_NORMED")